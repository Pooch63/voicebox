...
export default function Home() {
  ...
  const { appState, sessionStarted, endSession } = useAppStore();
  ...
  const handleStartListening = () => {
    if (!vad.listening && appState !== 'listening' && appState !== 'processing') {
      vad.start();
    }
  };
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-4 bg-[var(--background)] pb-20">
      ...
      <div className="flex flex-col items-center w-full max-w-4xl space-y-12 animate-fade-in">
        ...
        />
        <QuestionDisplay sessionActive={vad.listening} />
        <ResponseButtons />
        <TypeResponse />
      </div>
      <StatusBar />
