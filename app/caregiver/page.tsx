    ...
    return (
      <div className="space-y-6 sm:space-y-8 mt-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                {patients.map((p) => (
                  <option key={p.patient.user_id} value={p.patient.user_id}>
                    {p.patient.name || p.patient.user_id}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select
                value={newSchedule.pingType}
                onChange={(e) => setNewSchedule({ ...newSchedule, pingType: e.target.value as any })}
                className="w-full px-4 py-3 rounded-xl bg-[var(--background)] border border-[var(--border)] text-[var(--foreground)] focus:outline-none focus:border-[var(--primary)]"
              >
                <option value="hunger_check">Hunger Check</option>
                <option value="therapy_prompt">Therapy Prompt</option>
            ...
            <div>
              ...
              <input
                type="number"
                value={newSchedule.frequencyMinutes}
                onChange={(e) => setNewSchedule({ ...newSchedule, frequencyMinutes: parseInt(e.target.value) })}
                min={15}
                className="w-full px-4 py-3 rounded-xl bg-[var(--background)] border border-[var(--border)] text-[var(--foreground)] focus:outline-none focus:border-[var(--primary)]"
              />
              <p className="text-xs text-[var(--foreground)] opacity-60 mt-2">
                How often should the patient be pinged? (minimum 15 minutes)
              </p>
            </div>
            
            <button
              onClick={createSchedule}
              disabled={!newSchedule.patientId || savingSchedule}
              className="w-full sm:w-auto px-6 py-3 bg-[var(--primary)] text-white rounded-xl font-bold transition-all disabled:opacity-50 hover:opacity-90"
            >
        ...
        <section>
          ...
          {schedules.length === 0 ? (
            <div className="bg-[var(--surface)] rounded-2xl p-6 text-center border border-[var(--border)]/50">
              <p className="text-[var(--foreground)] opacity-60">
                No schedules created yet. Create one above to start periodic check-ins.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {schedules.map((schedule) => (
                <div
                  ...
                  className={`bg-[var(--surface)] rounded-xl p-5 border ${
                    schedule.active
                      ? 'border-green-500/50'
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">
                          {schedule.ping_type === 'hunger_check' && 'Hunger Check'}
                          {schedule.ping_type === 'therapy_prompt' && 'Therapy Prompt'}
                          {schedule.ping_type === 'wellness_check' && 'Wellness Check'}
                        </h3>
                      ...
                      <p className="text-sm text-[var(--foreground)] opacity-70">
                        Every {schedule.frequency_minutes} minutes
                      </p>
                      {schedule.next_ping_at && schedule.active && (
                        <p className="text-xs text-[var(--foreground)] opacity-50 mt-1">
                          Next ping: {new Date(schedule.next_ping_at).toLocaleString()}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => toggleSchedule(schedule.schedule_id, schedule.active)}
